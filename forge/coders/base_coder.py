...
from forge import __version__, models, prompts, urls, utils
...
from forge.io import ConfirmGroup, InputOutput
from forge.linter import Linter
from forge.llm import litellm
from forge.repo import ANY_GIT_ERROR, GitRepo
from forge.repomap import RepoMap
from forge.run_cmd import run_cmd
from forge.sendchat import RETRY_TIMEOUT, send_completion
from forge.utils import format_content, format_messages, format_tokens, is_image_file
...
class Coder:
    ...
    repo = None
    last_forge_commit_hash = None
    forge_edited_files = None
        ...
        **kwargs,
        ...
        if from_coder:
            ...
            update = dict(
                ...
                forge_commit_hashes=from_coder.forge_commit_hashes,
    ...
    def get_announcements(self):
        lines = []
        lines.append(f"forge v{__version__}")
        ...
        if self.repo:
            ...
            if num_files > 1000:
                lines.append(
                    "Warning: For large repos, consider using --subtree-only and .forgeignore"
                )
    ...
    def __init__(
        ...
        if forge_commit_hashes:
            self.forge_commit_hashes = forge_commit_hashes
        else:
            self.forge_commit_hashes = set()
        ...
        for fname in fnames:
            ...
            if self.repo and self.repo.ignored_file(fname):
                self.io.tool_warning(f"Skipping {fname} that matches forgeignore spec.")
                continue
    ...
    def send_message(self, inp):
        ...
        if edited:
            self.forge_edited_files.update(edited)
    ...
    def show_auto_commit_outcome(self, res):
        commit_hash, commit_message = res
        self.last_forge_commit_hash = commit_hash
        self.forge_commit_hashes.add(commit_hash)
        self.last_forge_commit_message = commit_message
        if self.show_diffs:
    ...
    def show_undo_hint(self):
        ...
        if self.commit_before_message[-1] != self.repo.get_head_commit_sha():
            self.io.tool_output("You can use /undo to undo and discard each forge commit.")
