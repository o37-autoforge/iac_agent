...
forge will report an error if a model responds indicating that
...
For more info: https://forge.chat/docs/token-limits.html
```
...
It's best to only add the files that forge will need to *edit*
...
as 4k tokens.
If you ask forge to make a large change that affects a lot
...
forge has been well tested when directly connecting to 
major 
[LLM provider cloud APIs](https://forge.chat/docs/llms.html).
For serving local models, 
[Ollama](https://forge.chat/docs/llms/ollama.html) is known to work well with forge.

Try using forge without an API proxy server
