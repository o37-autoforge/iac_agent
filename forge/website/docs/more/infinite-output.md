...
parent: More info
nav_order: 480
description: forge can handle "infinite output" from models that support prefill.
---
...
forge is able to work around this limit with models that support
"prefilling" the assistant response.
When you use forge with a model that supports prefill, you will see
...
When forge is collecting code edits from a model and
it hits the output token limit,
forge simply initiates another LLM request with the partial
response prefilled.
...
forge supports "infinite output" for models that support "prefill",
such as:
