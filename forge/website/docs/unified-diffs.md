...
forge now asks GPT-4 Turbo to use
...
forge's new "laziness" benchmark suite 
...
- **GPT-4 Turbo only scored 20% as a baseline** using forge's existing "SEARCH/REPLACE block" edit format. It outputs "lazy comments" on 12 of the tasks.
- **forge's new unified diff edit format raised the score to 61%**. Using this format reduced laziness by 3X, with GPT-4 Turbo only using lazy comments on 4 of the tasks.
...
- **The June GPT-4's baseline was 26%** using forge's existing "SEARCH/REPLACE block" edit format.
- **forge's new unified diff edit format raised June GPT-4's score to 59%**. 
...
forge's new unified diff editing format
...
numerous variations on forge's existing editing formats,
...
forge's new editing format and refactoring benchmark.
...
The design and implementation of forge's new unified diff editing format
...
forge's [previous benchmark results](https://forge.chat/docs/benchmarks.html) made
...
forge tells GPT not to include line numbers,
...
forge's system prompt encourages
...
When a patch fails, forge needs to ask GPT for a corrected version of the diff.
...
forge tries to be very flexible when applying diffs,
in order to handle defects.
If a hunk doesn't apply cleanly, forge uses a number of strategies:
...
**Experiments where flexible patching is disabled show a 9X increase in editing errors** on forge's original Exercism benchmark.

## Refactoring benchmark

forge has long used a
[benchmark suite based on 133 Exercism python exercises](https://forge.chat/2023/07/02/benchmarks.html).
...
[9 popular open source python repositories](https://github.com/forge-AI/refactor-benchmark)
...
A [simple python AST scanning script](https://github.com/forge-AI/forge/blob/main/benchmark/refactor_tools.py)
...
[benchmark suite that provokes, detects and quantifies GPT coding laziness](https://github.com/forge-AI/refactor-benchmark).
...
forge's new unified diff format seems
...
when originally building forge.
...
incorporated into forge provide
...
forge's simple, high level style of unified diffs.
