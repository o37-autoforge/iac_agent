...
forge no longer uses ctags to build a repo map.
Please see the newer article about
[using tree-sitter to build a better repo map](https://forge.chat/docs/repomap.html).
...
To address these issues, `forge` now
...
[chat transcript](https://forge.chat/examples/add-test.html)
...
To code with GPT-4 using the techniques discussed here:
  ...
  - Install [forge](https://forge.chat/docs/install.html).
  - Install universal ctags.
  - Run `forge` inside your repo, and it should say "Repo-map: universal-ctags using 1024 tokens".
...
This works pretty well, and is supported by `forge` -- you
...
The latest version of `forge` sends a **repo map** to GPT along with
...
Here's a
sample of the map of the forge repo, just showing the maps of
[main.py](https://github.com/forge-AI/forge/blob/main/forge/main.py)
and
[io.py](https://github.com/forge-AI/forge/blob/main/forge/io.py)
:
...
Mapping out the repo like this provides some benefits:
  ...
  - If it needs to see more code, GPT can use the map to figure out by itself which files it needs to look at. GPT will then ask to see these specific files, and `forge` will automatically add them to the chat context (with user approval).
...
Sending an appropriate subset of the repo map would help `forge` work
