...
parent: Usage
nav_order: 800
description: Tell forge to follow your coding conventions when it works on your code.
---
...
The easiest way to do that with forge is to simply create
...
and then we can add it to the forge chat, along with the file(s)
that we want to edit.

It's best to load the conventions file with `/read CONVENTIONS.md` 
or `forge --read CONVENTIONS.md`. 
This way it is marked as read-only, and cached if prompt caching
is enabled.

## Always load conventions

You can also configure forge to always load your conventions file
in the [`.forge.conf.yml` config file](https://forge.chat/docs/config/forge_conf.html):
...
> $ forge --read CONVENTIONS.md useragent.py  
>  
> forge v0.24.2-dev  
