...
forge has a few different chat modes:

- `code` - forge will make changes to your code to satisfy your requests.
- `architect` - forge will first propose a solution, then ask if you want it to turn that proposal into edits to your files.
- `ask` - forge will answer questions about your code, but never edit it.
- `help` - forge will answer questions about using forge, configuring, troubleshooting, etc.

By default, forge starts in "code" mode. As you are talking, you can
...
Or you can launch forge in one of the modes with the `--chat-mode <mode>` switch.
There is also a special shortcut `--architect` to launch in `--chat-mode architect`.

## Architect mode and the editor model

When you are in architect mode, forge sends your request to two models:
...
After the main model replies, forge will offer to edit the files based on the response.

2. To edit the files, forge sends a second LLM request asking for specific code editing instructions.
This request goes to the "editor" model.
forge has built in defaults to select an editor model based on your main model.
...
The editor model uses one of forge's edit formats to let the LLM
edit source files.
forge will pick a suitable edit format by default,
...
See this article on 
[forge's architect/editor mode](/2024/09/26/architect.html)
for more details.
...
Run `forge --model ollama/<ollama-model>`.
See these docs for more info: https://forge.chat/docs/llms/ollama.html
