...
In most cases, you can safely ignore forge's warning about unknown context
window size and model costs.

But, you can register context window limits and costs for models that aren't known
to forge. Create a `.forge.model.metadata.json` file in one of these locations:

- Your home directory.
- The root if your git repo.
- The current directory where you launch forge.
...
in the `.forge.model.metadata.json` file.
For example, use `deepseek/deepseek-chat`, not just `deepseek-chat`.

## Model settings

forge has a number of settings that control how it works with
different models.
...
a model that forge doesn't know about.

To do that,
create a `.forge.model.settings.yml` file in one of these locations:
...
[models.py](https://github.com/forge-AI/forge/blob/main/forge/models.py)
file for more details about all of the model setting that forge supports.

<!--[[[cog
from forge.models import get_model_settings_as_yaml
cog.out("```yaml\n")
