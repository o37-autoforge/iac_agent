...
from forge import __version__, utils
...
def get_package_files():
    for path in importlib_resources.files("forge.website").iterdir():
        if path.is_file():
            yield path
        elif path.is_dir():
...
def fname_to_url(filepath):
    ...
    return f"https://forge.chat/{url_path}"
...
def get_index():
    ...
    dname = Path.home() / ".forge" / "caches" / ("help." + __version__)
    ...
    if index is None:
        ...
        for fname in get_package_files():
            ...
            doc = Document(
                text=importlib_resources.files("forge.website")
                .joinpath(fname)
