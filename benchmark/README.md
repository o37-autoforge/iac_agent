...
# forge code editing benchmark harness

forge uses a "code editing" benchmark to quantitatively measure how well it works
...
See [this writeup for a longer discussion about the benchmark and how to interpret the results](https://forge.chat/docs/benchmarks.html).
...
There are 3 main tasks involved in benchmarking forge:
...
# Create the scratch dir to hold benchmarking results inside the main forge dir:
cd forge
...
# Inside the container, install forge as a development build.
...
- `--model` is the name of the model, same as you would pass directly to `forge`.
- `--edit-format` is the name of the edit format, same as you would pass directly to `forge`. When working with an experimental LLM, I recommend starting with `whole`
...
[forge leaderboard data files](https://github.com/forge-AI/forge/blob/main/forge/website/_data/).
...
[forge leaderboard data files](https://github.com/forge-AI/forge/blob/main/forge/website/_data/).
- These scripts are not intended for use by typical forge end users.
