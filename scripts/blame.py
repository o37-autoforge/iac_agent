    ...
    for file in files:
        ...
        if file_counts:
            ...
            for author, count in file_counts.items():
                ...
                if "(forge)" in author.lower():
                    forge_total += count

    total_lines = sum(grand_total.values())
    forge_percentage = (forge_total / total_lines) * 100 if total_lines > 0 else 0

    end_date = get_tag_date(end_tag if end_tag else "HEAD")

    return all_file_counts, grand_total, total_lines, forge_total, forge_percentage, end_date
...
def get_commit_authors(commits):
    ...
    for commit in commits:
        ...
        if commit_message.lower().startswith("forge:"):
            author += " (forge)"
...
def process_all_tags_since(start_tag):
    ...
    for i in tqdm(range(len(tags) - 1), desc="Processing tags"):
        ...
        all_file_counts, grand_total, total_lines, forge_total, forge_percentage, end_date = blame(
            start_tag, end_tag
        )
        results.append(
            {
                ...
                "forge_percentage": round(forge_percentage, 2),
            }
        )
    return results
...
def main():
    parser = argparse.ArgumentParser(description="Get forge/non-forge blame stats")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Find all tags since the specified tag and print forge percentage between each pair of"
            " successive tags"
        ),
    )
    parser.add_argument(
    ...
    if not args.all_since:
        print(f"- forge wrote {round(forge_percentage)}% of the code in this release.")
